...
// @mui
import { TextField, MenuItem, Select, InputLabel, FormControl, Box, SelectChangeEvent, CardHeader, Grid, Theme } from "@mui/material";
...
// Types
import { ICategory, ICategoryTableFilters, ICategoryTableFilterValue } from 'src/types/category';
...
export default function CategoryListView() {
  ...
  const [loadUpload, setLoadUpload] = useState(false); // Loading state for button
  const [uploadSuccess, setUploadSuccess] = useState(false); // To check if upload is successful
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      ? API_ENDPOINTS.category.update(currentRecordId ?? "") 
      : API_ENDPOINTS.category.new;
    
      const result = await sendData(accessToken, endpoint, formDataReq);
    
      setFormData({ name: "", description: "", main_category: "",icon_id:"" });
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      ...
      const paramSend = {
        ...
        const response = await fetchData(accessToken,API_ENDPOINTS.category.all,paramSend);
  ...
  const getData = useCallback(async () => {
    try {
      const result = await fetchData(accessToken, API_ENDPOINTS.category.list);
      setData(result?.data ?? []);
    } catch (err) {
      setError(err);
    } finally {
  ...
  const TABLE_HEAD = useMemo(() => [
    ...
    { id: 'name', label: t('category_table.name') ?? "Name", width:200 },
    { id: 'description', label: t('category_table.description') ?? "Description", width:250 },
    { id: 'status', label: t('category_table.status') ?? "Status", width: 150 },
      ...
      const onSuccess = (response: UploadResponse) => {
        ...
        fetch(HOST_API + API_ENDPOINTS.upload.complete, {
          ...
          .then((finalizeResponse) => {
            ...
            setFormData((prev) => ({ ...prev, icon_id: finalizeResponse?.data?.unique_key }));
          })
          .catch((error) => {
  ...
  const handleDeleteRow = useCallback(
    async (id: string) => {
      try {
        const endpoint = API_ENDPOINTS.category.delete(id);
    ...
    [dataInPage.length, table, tableData,getData,showToast,accessToken]
  );
  ...
  const handleEditRow = (edtiCategoryData: ICategory) => {
    setIsEditMode(true);
    setCurrentRecordId(edtiCategoryData?.unique_key);
    setFormData({
      name: edtiCategoryData.name,
      description: edtiCategoryData?.description,
      main_category: edtiCategoryData?.main_category?.unique_key,
      icon_id: "", // You can handle image differently if needed (e.g., image URL, file)
    });
    setPreview(edtiCategoryData?.icon?.path); // Assuming image is passed as a URL
  };

  const handleUpdateStatus = useCallback(
    async (id: string) => {
      try {
        const endpoint = API_ENDPOINTS.category.status(id);
  ...
  return (
    <>
      ...
      <Container maxWidth={settings.themeStretch ? false : 'lg'}>
        <CustomBreadcrumbs
          heading={t('category_section.head_title') ?? "Category"}
          links={[
            ...
            { name: t('category_section.section_name') ?? "List" },
          ]}
          sx={{
            mb: { xs: 3, md: 5 } ,
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '16px' }}>
            ...
            <div>
              <Card>
                <Grid xs={12} md={8}>
                  <CardHeader
                    title={isEditMode ? "Edit Details" : t('category_section.add_category') ?? "Add category"}
                  ...
                  <Box component="form" onSubmit={handleSubmit} sx={{p:2}}>
                    ...
                    <TextField
                      fullWidth
                      label={t('category_section.name') ?? "Name Category"}
                      name="name"
                    ...
                    <TextField
                      fullWidth
                      label={t('category_section.description') ?? "Description"}
                      name="description"
                    ...
                    <FormControl fullWidth margin="normal">
                      <InputLabel>{t('category_section.main_cat') ?? "Sub Category"}</InputLabel>
                      <Select name="main_category" value={formDataReq.main_category} onChange={handleSelectChange}>
                        <MenuItem>{t('category_section.select_main') ?? "Select Main Category"}</MenuItem>
                        {categories.map((cat) => (
                          <MenuItem key={cat.unique_key} value={cat.unique_key}>
                            {cat.name}
                          </MenuItem>
                        ))}
                      </Select>
                    ...
                    {/* <Button
                      ...
                      type="submit"
                      fullWidth
                      disabled={loadUpload || !uploadSuccess} // Disable until loading is complete
                      sx={{ mt: 2 }}
                    >
                      {loadUpload ? t('category_section.upload') ?? "Uploading..." : uploadSuccess ? t('category_section.submit') : t('category_section.submit')}
                    </Button> */}
                    ...
                    >
                      {(() => {
                        if (loadUpload) {
                          return t('category_section.upload') ?? "Uploading...";
                        }
                        if (isEditMode) {
                          return t('category_section.save') ?? "Save"; // Show "Save" in edit mode
                        }
                        return uploadSuccess ? t('category_section.submit') : t('category_section.submit');
                      })()}
                    </Button>
