...
// types
import { ISliderTableFilters, ISliderTableFilterValue } from 'src/types/slider';
// components
...
type Props = StackProps & {
  ...
  onFilters: (title: string, value: ISliderTableFilterValue) => void;
  //
...
export default function SliderTableFiltersResult({
  filters,
  onFilters,
  //
  onResetFilters,
    ...
    <Stack spacing={1.5} {...other}>
      ...
      {results === 0 ? (
          <Box component="span" sx={{ color: 'text.secondary', ml: 0.25 }}>
            {t('category_table.result_not') ?? "No result found"}
          </Box>
        ) : (
          <>
            <strong>{results}</strong>
            <Box component="span" sx={{ color: 'text.secondary', ml: 0.25 }}>
              {t('category_table.result_found') ?? "Result found:"}
            </Box>
          </>
        )}
      </Box>
      ...
      <Stack flexGrow={1} spacing={1} direction="row" flexWrap="wrap" alignItems="center">
        ...
        {!!filters.publish.length && (
          <Block label={t('category_table.publish') ?? "Publish:"}>
