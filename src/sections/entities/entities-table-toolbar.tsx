...
// types
import { EntitiesFilters, EntitiesFilterValue } from 'src/types/entities';
// components
...
type Props = {
  ...
  onFilters: (title: string, value: EntitiesFilterValue) => void;
  //
  publishOptions: {
    value: number;
    label: string;
...
export default function EntitiesTableToolbar({
  filters,
  onFilters,
  //
  publishOptions,
  ...
  const handleFilterPublish = useCallback(
    (event: SelectChangeEvent<number[]>) => {
      const value = Array.isArray(event.target.value) 
        ? event.target.value.join(',')  // Convert array to a comma-separated string
        : event.target.value;
  
      onFilters('publish', value);
    },
    [onFilters]
  );
