...
// types
import { IPostItem, IPostFilters } from 'src/types/post';
// _mock
// components
...
export default function PostListView() {
  ...
  const getData = useCallback(async () => {
    try {
      ...
      const link = API_ENDPOINTS.post.list("podcasts");
  ...
  const handleDeleteRow = async (uniqueKey: string) => {
    try {
      const endpoint = API_ENDPOINTS.post.delete(uniqueKey);
  ...
  return (
    <>
      ...
      >
        <CustomBreadcrumbs
          heading={t('section.list_post') ?? "Pots List"}
          links={[
          ...
          <Grid container spacing={2}>
            {dataFiltered.map((post, index) => (
              <Grid item xs={12} sm={6} key={post.unique_key}>
                <PostItemHorizontal post={post} onDeleteRow={handleDeleteRow} />
              </Grid>
            ))}
          </Grid>
      </Container>
